...
struct ReqGradeChangeDTO
{
    std::string userId;
    std::string targetGrade; // "VIP", "Gold", "일반" 등

    NLOHMANN_DEFINE_TYPE_INTRUSIVE(ReqGradeChangeDTO, userId, targetGrade)
};

struct ResGradeChangeDTO
{
    int status;
    std::string message;
    std::string currentGrade;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE(ResGradeChangeDTO, status, message, currentGrade)
};